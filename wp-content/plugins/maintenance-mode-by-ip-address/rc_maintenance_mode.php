...
*	 Plugin URI: https://rcit.consulting/
...
*	 Author URI: https://rcit.consulting/
*/
...
function rc_mmip_activation(){
	...
	//if options exist, check if it is empty. If it is, then create the variables
	if(!$rc_mmip_options){
		...
		//Mailchimp Settings
		$rc_mmip_options['rc_mmip_mchimp_section_title'] = 'Let me know when you launch';
		$rc_mmip_options['rc_mmip_mchimp_opt'] = 0;
		$rc_mmip_options['rc_mmip_mchimp_api'] = '';
		$rc_mmip_options['rc_mmip_mchimp_list'] = '';
		$rc_mmip_options['rc_mmip_mchimp_after_msg'] = 'Thank you for subscribing to our mailing list.';

		//Google reCaptcha Settings
		$rc_mmip_options['rc_mmip_mchimp_gcaptcha'] = 0;
		$rc_mmip_options['rc_mmip_mchimp_gcaptcha_site'] = '';
		$rc_mmip_options['rc_mmip_mchimp_gcaptcha_secret'] = '';

		//Social Profiles	
		$rc_mmip_options['rc_mmip_social'] = '';
		$rc_mmip_options['rc_mmip_deleteondeact'] = 1;
...
function rc_mmip_process_form(){
	...
	if(isset($_POST['rc_maint_mode_form_submitted'])){
		...
		if($hidden_field == 'Y'){
			...
			if(isset($_POST['rc_mmip_mchimp_section_title'])){
				$rc_mmip_mchimp_section_title = sanitize_text_field($_POST['rc_mmip_mchimp_section_title']);
			}else{
				$rc_mmip_mchimp_section_title = "let me know when you launch";
			}
			
			if(isset($_POST['rc_mmip_mchimp_after_msg'])){
				$rc_mmip_mchimp_after_msg = $_POST['rc_mmip_mchimp_after_msg'];	
			}else{
			...
			if($_POST['rc_mmip_mchimp_gcaptcha'] == 1 && $_POST['rc_mmip_mchimp_opt'] == 1 && isset($_POST['rc_mmip_mchimp_gcaptcha_site']) && $_POST['rc_mmip_mchimp_gcaptcha_site'] != ''){
					
						
						$_POST['rc_mmip_mchimp_gcaptcha_site'] = sanitize_text_field($_POST['rc_mmip_mchimp_gcaptcha_site']);
						$_POST['rc_mmip_mchimp_gcaptcha_secret'] = sanitize_text_field($_POST['rc_mmip_mchimp_gcaptcha_secret']);
						
						$rc_mmip_mchimp_gcaptcha = 1;
						$rc_mmip_mchimp_gcaptcha_site = $_POST['rc_mmip_mchimp_gcaptcha_site'];
						$rc_mmip_mchimp_gcaptcha_secret = $_POST['rc_mmip_mchimp_gcaptcha_secret'];
			...
			if(isset($_POST['rc_maint_mode_deleteondeact'])){
				$rc_mmip_deleteondeact = (int) $_POST['rc_maint_mode_deleteondeact'];
			}else{
			...
			//Mailchimp Settings
			$rc_mmip_options['rc_mmip_mchimp_section_title'] = $rc_mmip_mchimp_section_title;
			...
			$rc_mmip_options['rc_mmip_mchimp_after_msg'] = $rc_mmip_mchimp_after_msg;
			
			//Google reCaptcha Settings
			$rc_mmip_options['rc_mmip_mchimp_gcaptcha'] = $rc_mmip_mchimp_gcaptcha;
			$rc_mmip_options['rc_mmip_mchimp_gcaptcha_site'] = $rc_mmip_mchimp_gcaptcha_site;
			$rc_mmip_options['rc_mmip_mchimp_gcaptcha_secret'] = $rc_mmip_mchimp_gcaptcha_secret;

			//Social Profiles
			$rc_mmip_options['rc_mmip_social'] = $rc_mmip_social;
			
			//Delete settings on deactivation
			$rc_mmip_options['rc_mmip_deleteondeact'] = $rc_mmip_deleteondeact;
...
function rc_maint_mchimp_action(){
	...
	if($rc_mmip_mchimp_gcaptcha == 1){
		...
		$secret = $rc_mmip_options['rc_mmip_mchimp_gcaptcha_secret'];
		$recaptcha = new \ReCaptcha\ReCaptcha($secret);
		$resp = $recaptcha->verify($_POST['gcaptcha'], $_SERVER['REMOTE_ADDR']);
	...
	if($_POST['email'] == ''){
		...
		echo 'An Invalid Email Address has been provided. Try again.';
		wp_die();
		
	}else if ($rc_mmip_mchimp_gcaptcha == 1 && !($resp->isSuccess())){
	...
	if($receivedJson['status'] == 'subscribed'){
		echo '<i class="fa fa-check"></i> ' . esc_html($rc_mmip_mchimp_after_msg);
...
add_action('wp_enqueue_scripts','check_scripts', 99999);
...
function check_scripts(){
  ...
  if ( in_array('bootstrap.css', $srcs) || in_array('bootstrap.min.css', $srcs)  ) {
    /* echo 'font-awesome.css registered'; */
		...
		wp_enqueue_style('rc_maint_frontend_bootstrap_css',  plugin_dir_url( __FILE__ ) . 'templates/assets/bootstrap/bootstrap.min.css');
  }
