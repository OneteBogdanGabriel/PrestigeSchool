...
?>
	<div class="wordapp-footer">
		<ul class="ui-navbar" style="min-height: 51px;">
	...
	<?php
		...
		foreach ((array) $menu_items as $key => $menu_item) {
			...
			if (!isset($varMenu['bottomIcon'][$i])) {
				...
				$imgicon = '<img src="'.WORDAPP_DIR_URL.'themes/MyTheme/icons-png/'.$varMenu['bottomIcon'][$i].'-white.png">';
			}
			if (!isset($varMenu['bottomIcon'][$i])) {
			$varMenu['bottomIcon'][$i] = '';
			}
?>
       ...
       <li data-filtertext="wai-aria voiceover accessibility screen reader" class="ui-block-a">
							<a class="bottomBar" href="<?php echo $thedddURL;
			?>" <?php echo $target;
...
@import url(http://fonts.googleapis.com/css?family=<?php echo $varColor['ColorTextFont']; ?>);
@import url(http://fonts.googleapis.com/css?family=<?php echo $varColor['ColorTextFontHHH']; ?>);
@import url(http://fonts.googleapis.com/css?family=<?php echo $varColor['ColorTextFontP']; ?>);
...
<script src="<?php echo  WORDAPP_DIR_URL; ?>themes/MyTheme/js/iosOverlay.js"></script>
  <script src="<?php echo  WORDAPP_DIR_URL; ?>themes/MyTheme/js/spin.min.js"></script>
           ...
           <script type="text/javascript" src="https://s3.amazonaws.com/cordovajs/ng-cordova/cordova/<?php echo $devCordova; ?>/js/indexMyTheme.js?<?php echo date('Ymdhsi')?>"></script>
